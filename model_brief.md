...
**Generado (UTC, ISO 8601):** 2026-09-22T05:03:31Z
**Caducidad (UTC, ISO 8601):** 2026-09-23T17:03:32Z -- TTL=36h, FIJADO por Carlos el 2026-09-10 (ver src/run_models.py, TTL_BRIEF_HORAS, para el argumento completo y la condicion de reapertura). Pasada esta fecha, este documento NO debe usarse para decidir sin verificar antes que la publicacion de hoy funciono.
...
|---|---|---|---|---|---|---|
| MS-VAR | frac 20d en estres | — | 0.50 | no | 705 | retirado · rodaje |
| MS-VAR (largo) | estres confirmado >=2d (hoy) | 0.000 | 0.50 | no | 9164 | ok · rodaje |
| BVAR-SV | P(sigma_T > q90) | 0.013 | 0.35 | no | 701 | ok · rodaje |
| cDCC | pctl_corr (NO prob.) | 0.847 | 0.90 | no | 705 | ok · rodaje |
| GARCH-t | extremeza BTC (2 colas) | 0.968 | 0.90 | SI | 7 | ok · rodaje |
...
MSH-VAR(1) sobre `r_NASDAQCOM`, `d_VIXCLS`, `d_BAA10Y`, PANEL_LARGO (T=9164d, muestra macro sin BTC). Estimado por EM (Hamilton-Kim), no por MLE directa -ver models/msvar.py.

| Ajuste | Valor |
|---|---|
| log-verosimilitud | -6918.7 |
| parametros | 29 |
| AIC / BIC | 13895.4 / 14101.9 |
| convergencia (EM) | si |
| iteraciones EM | 32 |
...
|---|---|---|---|---|
| calma | 0.939 | 16.5d | 0.724 | 1.238e-04 |
...
Ratio |Sigma| estres/calma: **388.9x** (minimo 3). Duracion del regimen de estres: **6.3d** (minimo 5).

**Voto de hoy (histeresis de 2d):** sin confirmar -probabilidad suavizada de hoy: 0.042. La histeresis es una capa de LECTURA sobre la probabilidad ya calculada (no cambia el filtro ni la estimacion) -motivada por que el 52.23% de las probabilidades no nitidas de este panel son tramos aislados de mediana 2 dias, no ambiguedad estructural sostenida (ver models/msvar.py).
...
|---|---|---|
| r_NASDAQCOM / d_VIXCLS | -0.688 | -0.704 |
...
| ESS de sigma_T | 450.0 |
| ESS minimo de phi | 22.8 |
...
| Medida | Valor |
|---|---|
| sigma_T (media posterior) | 1.122 |
| sd posterior de sigma_T | 0.301 |
| IC 90% de sigma_T | [0.72, 1.67] |
| mediana de la trayectoria | 1.324 |
| cociente sigma_T / mediana | 0.85 |
| P(sigma_T > q90) +/- MCSE | 0.013 +/- 0.003 |
...
|---|---|---|---|
| r_BTCUSDT | 0.904 | 0.068 | 27 |
| r_NASDAQ100 | 0.959 | 0.040 | 44 |
| d_DGS10 | 0.955 | 0.019 | 23 |
...
| pctl_corr (hoy) | 0.847 |
| rho_avg (hoy) | -0.134 |
| rho_avg (mediana historica) | -0.149 |
...
| r_BTCUSDT / d_BAMLH0A0HYM2 | -0.197 |
| r_NASDAQ100 / d_BAMLH0A0HYM2 | -0.504 |
...
|---|---|---|---|---|---|
| BTC | 1032 | 4.35 | cola pesada | 0.984 | 2.64 |
| ETH | 1032 | 3.75 | cola pesada | 0.821 | 2.66 |
| BNSOL | 1032 | 6.90 | cola pesada | 0.889 | 2.54 |
| BNB | 1032 | 4.23 | cola pesada | 0.805 | 2.64 |
| PAXG | 1032 | 3.66 | cola pesada | 0.342 | 2.66 |
| SPYB | 8467 | 6.46 | cola pesada | 0.981 | 2.55 |
| SMHB | 6612 | 9.48 | cola pesada | 0.957 | 2.48 |
...
| CISS (BCE) | 2026-09-18 | 0.034 | 31% | 0.002 | 7239 |
...
- **MS-VAR (largo)**: EM (Hamilton-Kim), identificado: dispersion entre arranques 0.00%, |Sigma| ratio 388.9x, duracion 6.3d. Vota con histeresis de 2d sobre p>0.5 -ver 'REGLA DE HISTERESIS' en models/msvar.py (validado: RCM=17.39, alineacion 6/6 episodios de estres historicos, sensibilidad de A 0.033<0.05). p_suavizada de hoy=0.0419, confirmado_estres_hoy=no (2d consecutivos).
- **BVAR-SV**: P(sigma_T > q90 de su propia trayectoria); nula=0.10. sigma_T=1.12 vs mediana 1.32, persistencia phi=0.90
- **cDCC**: pctl_corr=0.847 (rango percentil, NO probabilidad); rho_hoy(pares)=[0.3, -0.2, -0.5], persistencia_dcc=0.990 — persistencia_dcc=0.990 > 0.98: correlacion casi integrada (analogo del IGARCH). Puede senalar un cambio de regimen en la correlacion no modelado, o ser artefacto de muestra corta -no hay evidencia aqui de cual; no se corrige. Leer rho_hoy/pctl_corr con cautela.
- **GARCH-t**: p_stress = extremeza de dos colas de BTC (|2*hoy_percentil-1|); hoy_percentil BTC=0.984; proxies: SPYB<-SPY, SMHB<-SMH
