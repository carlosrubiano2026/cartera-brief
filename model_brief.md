...
**Generado (UTC, ISO 8601):** 2026-09-11T05:03:56Z
...
- **MS-VAR**: 2026-08-22 (0.7 meses, EN RODAJE)
- **MS-VAR (largo)**: 2026-08-22 (0.7 meses, EN RODAJE)
- **BVAR-SV**: 2026-08-22 (0.7 meses, EN RODAJE)
- **cDCC**: 2026-08-23 (0.6 meses, EN RODAJE)
- **GARCH-t**: 2026-08-23 (0.6 meses, EN RODAJE)
...
|---|---|---|---|---|---|---|
| MS-VAR | frac 20d en estres | — | 0.50 | no | 698 | retirado · rodaje |
| MS-VAR (largo) | estres confirmado >=2d (hoy) | 0.000 | 0.50 | no | 9157 | ok · rodaje |
| BVAR-SV | P(sigma_T > q90) | 0.039 | 0.35 | no | 694 | ok · rodaje |
| cDCC | pctl_corr (NO prob.) | 0.683 | 0.90 | no | 698 | ok · rodaje |
| GARCH-t | extremeza BTC (2 colas) | 0.675 | 0.90 | no | 7 | ok · rodaje |
...
MSH-VAR(1) sobre `r_NASDAQCOM`, `d_VIXCLS`, `d_BAA10Y`, PANEL_LARGO (T=9157d, muestra macro sin BTC). Estimado por EM (Hamilton-Kim), no por MLE directa -ver models/msvar.py.

| Ajuste | Valor |
|---|---|
| log-verosimilitud | -6912.5 |
| parametros | 29 |
| AIC / BIC | 13883.0 / 14089.6 |
| convergencia (EM) | si |
| iteraciones EM | 32 |
...
|---|---|---|---|---|
| calma | 0.939 | 16.4d | 0.723 | 1.230e-04 |
| estres | 0.841 | 6.3d | 0.277 | 4.796e-02 |
...
Ratio |Sigma| estres/calma: **390.0x** (minimo 3). Duracion del regimen de estres: **6.3d** (minimo 5).

**Voto de hoy (histeresis de 2d):** sin confirmar -probabilidad suavizada de hoy: 0.008. La histeresis es una capa de LECTURA sobre la probabilidad ya calculada (no cambia el filtro ni la estimacion) -motivada por que el 52.23% de las probabilidades no nitidas de este panel son tramos aislados de mediana 2 dias, no ambiguedad estructural sostenida (ver models/msvar.py).
...
| ESS de sigma_T | 642.7 |
| ESS minimo de phi | 16.0 |
...
| Medida | Valor |
|---|---|
| sigma_T (media posterior) | 1.232 |
| sd posterior de sigma_T | 0.348 |
| IC 90% de sigma_T | [0.75, 1.85] |
| mediana de la trayectoria | 1.302 |
| cociente sigma_T / mediana | 0.95 |
| P(sigma_T > q90) +/- MCSE | 0.039 +/- 0.005 |
...
|---|---|---|---|
| r_BTCUSDT | 0.841 | 0.120 | 16 |
| r_NASDAQ100 | 0.960 | 0.040 | 96 |
| d_DGS10 | 0.965 | 0.016 | 33 |
...
| r_BTCUSDT / d_BAMLH0A0HYM2 | -0.229 |
| r_NASDAQ100 / d_BAMLH0A0HYM2 | -0.506 |
...
|---|---|---|---|---|---|
| BTC | 1021 | 4.36 | cola pesada | 0.162 | 2.64 |
| ETH | 1021 | 3.73 | cola pesada | 0.359 | 2.66 |
| BNSOL | 1021 | 6.77 | cola pesada | 0.212 | 2.54 |
| BNB | 1021 | 4.21 | cola pesada | 0.336 | 2.64 |
| PAXG | 1021 | 3.70 | cola pesada | 0.066 | 2.66 |
| SPYB | 8460 | 6.45 | cola pesada | 0.144 | 2.55 |
| SMHB | 6605 | 9.46 | cola pesada | 0.111 | 2.48 |
...
| CISS (BCE) | 2026-09-09 | 0.036 | 32% | 0.009 | 7232 |
...
- **MS-VAR (largo)**: EM (Hamilton-Kim), identificado: dispersion entre arranques 0.00%, |Sigma| ratio 390.0x, duracion 6.3d. Vota con histeresis de 2d sobre p>0.5 -ver 'REGLA DE HISTERESIS' en models/msvar.py (validado: RCM=17.39, alineacion 6/6 episodios de estres historicos, sensibilidad de A 0.033<0.05). p_suavizada de hoy=0.0078, confirmado_estres_hoy=no (2d consecutivos).
- **BVAR-SV**: P(sigma_T > q90 de su propia trayectoria); nula=0.10. sigma_T=1.23 vs mediana 1.30, persistencia phi=0.84
- **cDCC**: pctl_corr=0.683 (rango percentil, NO probabilidad); rho_hoy(pares)=[0.31, -0.23, -0.51], persistencia_dcc=0.991 — persistencia_dcc=0.991 > 0.98: correlacion casi integrada (analogo del IGARCH). Puede senalar un cambio de regimen en la correlacion no modelado, o ser artefacto de muestra corta -no hay evidencia aqui de cual; no se corrige. Leer rho_hoy/pctl_corr con cautela.
- **GARCH-t**: p_stress = extremeza de dos colas de BTC (|2*hoy_percentil-1|); hoy_percentil BTC=0.162; proxies: SPYB<-SPY, SMHB<-SMH
