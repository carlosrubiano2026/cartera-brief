...
**Generado (UTC, ISO 8601):** 2026-09-19T05:03:17Z
**Caducidad (UTC, ISO 8601):** 2026-09-20T17:03:17Z -- TTL=36h, FIJADO por Carlos el 2026-09-10 (ver src/run_models.py, TTL_BRIEF_HORAS, para el argumento completo y la condicion de reapertura). Pasada esta fecha, este documento NO debe usarse para decidir sin verificar antes que la publicacion de hoy funciono.
...
|---|---|---|---|---|---|---|
| MS-VAR | frac 20d en estres | — | 0.50 | no | 704 | retirado · rodaje |
...
| GARCH-t | extremeza BTC (2 colas) | 0.965 | 0.90 | SI | 7 | ok · rodaje |
...
MSH-VAR(1) sobre `r_NASDAQCOM`, `d_VIXCLS`, `d_BAA10Y`, PANEL_LARGO (T=9163d, muestra macro sin BTC). Estimado por EM (Hamilton-Kim), no por MLE directa -ver models/msvar.py.

| Ajuste | Valor |
|---|---|
| log-verosimilitud | -6918.4 |
| parametros | 29 |
| AIC / BIC | 13894.7 / 14101.3 |
| convergencia (EM) | si |
| iteraciones EM | 32 |
...
|---|---|---|---|---|
| calma | 0.939 | 16.5d | 0.724 | 1.237e-04 |
...
Ratio |Sigma| estres/calma: **389.0x** (minimo 3). Duracion del regimen de estres: **6.3d** (minimo 5).

**Voto de hoy (histeresis de 2d):** sin confirmar -probabilidad suavizada de hoy: 0.118. La histeresis es una capa de LECTURA sobre la probabilidad ya calculada (no cambia el filtro ni la estimacion) -motivada por que el 52.23% de las probabilidades no nitidas de este panel son tramos aislados de mediana 2 dias, no ambiguedad estructural sostenida (ver models/msvar.py).
...
| ESS de sigma_T | 611.5 |
| ESS minimo de phi | 16.9 |
...
| Medida | Valor |
|---|---|
| sigma_T (media posterior) | 1.107 |
| sd posterior de sigma_T | 0.284 |
| IC 90% de sigma_T | [0.73, 1.62] |
...
|---|---|---|---|
| r_BTCUSDT | 0.912 | 0.061 | 83 |
| r_NASDAQ100 | 0.955 | 0.043 | 17 |
| d_DGS10 | 0.962 | 0.016 | 23 |
...
| r_BTCUSDT / d_BAMLH0A0HYM2 | -0.196 |
...
|---|---|---|---|---|---|
| BTC | 1029 | 4.34 | cola pesada | 0.982 | 2.64 |
| ETH | 1029 | 3.74 | cola pesada | 0.966 | 2.66 |
| BNSOL | 1029 | 6.86 | cola pesada | 0.957 | 2.54 |
| BNB | 1029 | 4.20 | cola pesada | 0.713 | 2.64 |
| PAXG | 1029 | 3.68 | cola pesada | 0.485 | 2.66 |
...
- **GARCH-t**: p_stress = extremeza de dos colas de BTC (|2*hoy_percentil-1|); hoy_percentil BTC=0.982; proxies: SPYB<-SPY, SMHB<-SMH
