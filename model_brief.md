...
**Generado (UTC, ISO 8601):** 2026-09-02T05:03:40Z
...
|---|---|---|---|---|---|---|
| MS-VAR | frac 20d en estres | — | 0.50 | no | 692 | retirado · rodaje |
| MS-VAR (largo) | estres confirmado >=2d (hoy) | 0.000 | 0.50 | no | 9151 | ok · rodaje |
| BVAR-SV | P(sigma_T > q90) | 0.052 | 0.35 | no | 688 | ok · rodaje |
| cDCC | pctl_corr (NO prob.) | 0.597 | 0.90 | no | 692 | ok · rodaje |
| GARCH-t | extremeza BTC (2 colas) | 0.596 | 0.90 | no | 7 | ok · rodaje |
...
MSH-VAR(1) sobre `r_NASDAQCOM`, `d_VIXCLS`, `d_BAA10Y`, PANEL_LARGO (T=9151d, muestra macro sin BTC). Estimado por EM (Hamilton-Kim), no por MLE directa -ver models/msvar.py.

| Ajuste | Valor |
|---|---|
| log-verosimilitud | -6917.2 |
| parametros | 29 |
| AIC / BIC | 13892.4 / 14099.0 |
| convergencia (EM) | si |
| iteraciones EM | 34 |
...
|---|---|---|---|---|
| calma | 0.939 | 16.4d | 0.723 | 1.232e-04 |
| estres | 0.841 | 6.3d | 0.277 | 4.799e-02 |
...
**Voto de hoy (histeresis de 2d):** sin confirmar -probabilidad suavizada de hoy: 0.109. La histeresis es una capa de LECTURA sobre la probabilidad ya calculada (no cambia el filtro ni la estimacion) -motivada por que el 52.23% de las probabilidades no nitidas de este panel son tramos aislados de mediana 2 dias, no ambiguedad estructural sostenida (ver models/msvar.py).
...
| ESS de sigma_T | 597.6 |
| ESS minimo de phi | 28.2 |
...
| Medida | Valor |
|---|---|
| sigma_T (media posterior) | 1.307 |
| sd posterior de sigma_T | 0.322 |
| IC 90% de sigma_T | [0.86, 1.89] |
| mediana de la trayectoria | 1.346 |
| cociente sigma_T / mediana | 0.97 |
| P(sigma_T > q90) +/- MCSE | 0.052 +/- 0.006 |
...
|---|---|---|---|
| r_BTCUSDT | 0.922 | 0.050 | 28 |
| r_NASDAQ100 | 0.949 | 0.055 | 65 |
| d_DGS10 | 0.968 | 0.012 | 61 |
...
|---|---|---|---|---|---|
| BTC | 1012 | 4.37 | cola pesada | 0.202 | 2.63 |
| ETH | 1012 | 3.73 | cola pesada | 0.169 | 2.66 |
| BNSOL | 1012 | 6.79 | cola pesada | 0.156 | 2.54 |
| BNB | 1012 | 4.21 | cola pesada | 0.329 | 2.64 |
| PAXG | 1012 | 3.67 | cola pesada | 0.018 | 2.66 |
...
- **MS-VAR (largo)**: EM (Hamilton-Kim), identificado: dispersion entre arranques 0.00%, |Sigma| ratio 389.6x, duracion 6.3d. Vota con histeresis de 2d sobre p>0.5 -ver 'REGLA DE HISTERESIS' en models/msvar.py (validado: RCM=17.39, alineacion 6/6 episodios de estres historicos, sensibilidad de A 0.033<0.05). p_suavizada de hoy=0.1095, confirmado_estres_hoy=no (2d consecutivos).
- **BVAR-SV**: P(sigma_T > q90 de su propia trayectoria); nula=0.10. sigma_T=1.31 vs mediana 1.35, persistencia phi=0.92
- **cDCC**: pctl_corr=0.597 (rango percentil, NO probabilidad); rho_hoy(pares)=[0.31, -0.25, -0.51], persistencia_dcc=0.991 — persistencia_dcc=0.991 > 0.98: correlacion casi integrada (analogo del IGARCH). Puede senalar un cambio de regimen en la correlacion no modelado, o ser artefacto de muestra corta -no hay evidencia aqui de cual; no se corrige. Leer rho_hoy/pctl_corr con cautela.
- **GARCH-t**: p_stress = extremeza de dos colas de BTC (|2*hoy_percentil-1|); hoy_percentil BTC=0.202; proxies: SPYB<-SPY, SMHB<-SMH
