...
**Generado (UTC, ISO 8601):** 2026-09-16T05:03:17Z
**Caducidad (UTC, ISO 8601):** 2026-09-17T17:03:17Z -- TTL=36h, FIJADO por Carlos el 2026-09-10 (ver src/run_models.py, TTL_BRIEF_HORAS, para el argumento completo y la condicion de reapertura). Pasada esta fecha, este documento NO debe usarse para decidir sin verificar antes que la publicacion de hoy funciono.
...
|---|---|---|---|---|---|---|
| MS-VAR | frac 20d en estres | — | 0.50 | no | 701 | retirado · rodaje |
| MS-VAR (largo) | estres confirmado >=2d (hoy) | 0.000 | 0.50 | no | 9160 | ok · rodaje |
| BVAR-SV | P(sigma_T > q90) | 0.026 | 0.35 | no | 697 | ok · rodaje |
| cDCC | pctl_corr (NO prob.) | 0.756 | 0.90 | no | 701 | ok · rodaje |
| GARCH-t | extremeza BTC (2 colas) | 0.825 | 0.90 | no | 7 | ok · rodaje |
...
MSH-VAR(1) sobre `r_NASDAQCOM`, `d_VIXCLS`, `d_BAA10Y`, PANEL_LARGO (T=9160d, muestra macro sin BTC). Estimado por EM (Hamilton-Kim), no por MLE directa -ver models/msvar.py.

| Ajuste | Valor |
|---|---|
| log-verosimilitud | -6915.8 |
| parametros | 29 |
| AIC / BIC | 13889.6 / 14096.1 |
| convergencia (EM) | si |
| iteraciones EM | 32 |
...
Ratio |Sigma| estres/calma: **389.4x** (minimo 3). Duracion del regimen de estres: **6.3d** (minimo 5).

**Voto de hoy (histeresis de 2d):** sin confirmar -probabilidad suavizada de hoy: 0.032. La histeresis es una capa de LECTURA sobre la probabilidad ya calculada (no cambia el filtro ni la estimacion) -motivada por que el 52.23% de las probabilidades no nitidas de este panel son tramos aislados de mediana 2 dias, no ambiguedad estructural sostenida (ver models/msvar.py).
...
|---|---|---|---|
| r_BTCUSDT | 0.889 | 0.085 | 12 |
| r_NASDAQ100 | 0.959 | 0.041 | 21 |
| d_DGS10 | 0.961 | 0.016 | 18 |
...
| r_BTCUSDT / d_BAMLH0A0HYM2 | -0.215 |
...
|---|---|---|---|---|---|
| BTC | 1026 | 4.34 | cola pesada | 0.088 | 2.64 |
| ETH | 1026 | 3.73 | cola pesada | 0.078 | 2.66 |
| BNSOL | 1026 | 6.74 | cola pesada | 0.072 | 2.54 |
| BNB | 1026 | 4.19 | cola pesada | 0.324 | 2.64 |
| PAXG | 1026 | 3.67 | cola pesada | 0.665 | 2.66 |
| SPYB | 8463 | 6.45 | cola pesada | 0.207 | 2.55 |
| SMHB | 6608 | 9.46 | cola pesada | 0.514 | 2.48 |
...
| CISS (BCE) | 2026-09-14 | 0.038 | 33% | 0.012 | 7235 |
...
- **MS-VAR (largo)**: EM (Hamilton-Kim), identificado: dispersion entre arranques 0.00%, |Sigma| ratio 389.4x, duracion 6.3d. Vota con histeresis de 2d sobre p>0.5 -ver 'REGLA DE HISTERESIS' en models/msvar.py (validado: RCM=17.39, alineacion 6/6 episodios de estres historicos, sensibilidad de A 0.033<0.05). p_suavizada de hoy=0.0319, confirmado_estres_hoy=no (2d consecutivos).
- **BVAR-SV**: P(sigma_T > q90 de su propia trayectoria); nula=0.10. sigma_T=1.17 vs mediana 1.33, persistencia phi=0.89
- **cDCC**: pctl_corr=0.756 (rango percentil, NO probabilidad); rho_hoy(pares)=[0.3, -0.21, -0.51], persistencia_dcc=0.991 — persistencia_dcc=0.991 > 0.98: correlacion casi integrada (analogo del IGARCH). Puede senalar un cambio de regimen en la correlacion no modelado, o ser artefacto de muestra corta -no hay evidencia aqui de cual; no se corrige. Leer rho_hoy/pctl_corr con cautela.
- **GARCH-t**: p_stress = extremeza de dos colas de BTC (|2*hoy_percentil-1|); hoy_percentil BTC=0.088; proxies: SPYB<-SPY, SMHB<-SMH
