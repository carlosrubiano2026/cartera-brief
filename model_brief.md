...
**Generado (UTC, ISO 8601):** 2026-09-15T05:03:39Z
**Caducidad (UTC, ISO 8601):** 2026-09-16T17:03:39Z -- TTL=36h, FIJADO por Carlos el 2026-09-10 (ver src/run_models.py, TTL_BRIEF_HORAS, para el argumento completo y la condicion de reapertura). Pasada esta fecha, este documento NO debe usarse para decidir sin verificar antes que la publicacion de hoy funciono.
...
|---|---|---|---|---|---|---|
| MS-VAR | frac 20d en estres | — | 0.50 | no | 700 | retirado · rodaje |
| MS-VAR (largo) | estres confirmado >=2d (hoy) | 0.000 | 0.50 | no | 9159 | ok · rodaje |
| BVAR-SV | P(sigma_T > q90) | 0.022 | 0.35 | no | 696 | ok · rodaje |
| cDCC | pctl_corr (NO prob.) | 0.766 | 0.90 | no | 700 | ok · rodaje |
| GARCH-t | extremeza BTC (2 colas) | 0.120 | 0.90 | no | 7 | ok · rodaje |
...
MSH-VAR(1) sobre `r_NASDAQCOM`, `d_VIXCLS`, `d_BAA10Y`, PANEL_LARGO (T=9159d, muestra macro sin BTC). Estimado por EM (Hamilton-Kim), no por MLE directa -ver models/msvar.py.

| Ajuste | Valor |
|---|---|
| log-verosimilitud | -6915.6 |
| parametros | 29 |
| AIC / BIC | 13889.1 / 14095.7 |
| convergencia (EM) | si |
| iteraciones EM | 32 |
...
|---|---|---|---|---|
| calma | 0.939 | 16.5d | 0.724 | 1.233e-04 |
| estres | 0.841 | 6.3d | 0.276 | 4.804e-02 |
...
**Voto de hoy (histeresis de 2d):** sin confirmar -probabilidad suavizada de hoy: 0.080. La histeresis es una capa de LECTURA sobre la probabilidad ya calculada (no cambia el filtro ni la estimacion) -motivada por que el 52.23% de las probabilidades no nitidas de este panel son tramos aislados de mediana 2 dias, no ambiguedad estructural sostenida (ver models/msvar.py).
...
| ESS de sigma_T | 756.1 |
| ESS minimo de phi | 24.3 |
...
| Medida | Valor |
|---|---|
| sigma_T (media posterior) | 1.190 |
| sd posterior de sigma_T | 0.286 |
| IC 90% de sigma_T | [0.80, 1.71] |
| mediana de la trayectoria | 1.342 |
...
|---|---|---|---|
| r_BTCUSDT | 0.916 | 0.054 | 24 |
| r_NASDAQ100 | 0.960 | 0.040 | 74 |
| d_DGS10 | 0.960 | 0.016 | 67 |
...
| Par | rho_hoy |
|---|---|
| r_BTCUSDT / r_NASDAQ100 | 0.307 |
| r_BTCUSDT / d_BAMLH0A0HYM2 | -0.224 |
| r_NASDAQ100 / d_BAMLH0A0HYM2 | -0.501 |
...
|---|---|---|---|---|---|
| BTC | 1025 | 4.33 | cola pesada | 0.440 | 2.64 |
| ETH | 1025 | 3.72 | cola pesada | 0.322 | 2.66 |
| BNSOL | 1025 | 6.70 | cola pesada | 0.498 | 2.54 |
| BNB | 1025 | 4.20 | cola pesada | 0.346 | 2.64 |
| PAXG | 1025 | 3.68 | cola pesada | 0.208 | 2.66 |
| SPYB | 8462 | 6.45 | cola pesada | 0.216 | 2.55 |
| SMHB | 6607 | 9.47 | cola pesada | 0.014 | 2.48 |
...
| CISS (BCE) | 2026-09-11 | 0.033 | 30% | 0.007 | 7234 |
...
- **MS-VAR (largo)**: EM (Hamilton-Kim), identificado: dispersion entre arranques 0.00%, |Sigma| ratio 389.5x, duracion 6.3d. Vota con histeresis de 2d sobre p>0.5 -ver 'REGLA DE HISTERESIS' en models/msvar.py (validado: RCM=17.39, alineacion 6/6 episodios de estres historicos, sensibilidad de A 0.033<0.05). p_suavizada de hoy=0.0797, confirmado_estres_hoy=no (2d consecutivos).
- **BVAR-SV**: P(sigma_T > q90 de su propia trayectoria); nula=0.10. sigma_T=1.19 vs mediana 1.34, persistencia phi=0.92
- **cDCC**: pctl_corr=0.766 (rango percentil, NO probabilidad); rho_hoy(pares)=[0.31, -0.22, -0.5], persistencia_dcc=0.991 — persistencia_dcc=0.991 > 0.98: correlacion casi integrada (analogo del IGARCH). Puede senalar un cambio de regimen en la correlacion no modelado, o ser artefacto de muestra corta -no hay evidencia aqui de cual; no se corrige. Leer rho_hoy/pctl_corr con cautela.
- **GARCH-t**: p_stress = extremeza de dos colas de BTC (|2*hoy_percentil-1|); hoy_percentil BTC=0.440; proxies: SPYB<-SPY, SMHB<-SMH
